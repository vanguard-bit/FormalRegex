...
# ---------------------------
# Constraint helpers (same logic as before)
# ---------------------------
...
def resolve_single_constraint(expr, var):
    s = expr.strip()
    # chain: A op var op B
    chain = re.compile(rf"^\s*(\d+)\s*(<=|<|>=|>)\s*{var}\s*(<=|<|>=|>)\s*(\d+)\s*$")
    m = chain.match(s)
    if m:
...
def index():
    # initial page - do not precompute anything heavy here
...
def run():
    """
    Keep existing HTML POST endpoint for non-JS fallback and compatibility.
    It renders a full page like before (so nothing breaks).
    """
...
def api_run():
    """
    AJAX endpoint returning JSON. Accepts JSON body or form-encoded data.
    Returns:
      { translated, highlighted, accepted: [[line, bool], ...], error }
    """
    # Accept JSON payload or form POST (for compatibility)
    data = {}
    if request.is_json:
        data = request.get_json()
    else:
        # fallback to form data
        data["pattern"] = request.form.get("pattern", "")
        data["text"] = request.form.get("text", "")
        data["constraints"] = request.form.get("constraints", "")
    ...
    try:
        # Resolve numeric bounds and substitute into pattern
        var_bounds = resolve_constraints(constraints, pattern)
        parsed_pattern = substitute_vars(pattern, var_bounds)

        # Parse educational RE into AST
        ast = Parser(parsed_pattern).parse()

        # Translate AST to python regex string
        py_regex = ast_to_regex(ast)

        # compile to validate regex and catch errors early
        try:
            re.compile(py_regex)
            ...
            return jsonify(
                {
                    ...
                    "highlighted": "",
                    "accepted": [],
                    "error": f"Invalid generated regex: {rex}",
                }
            ), 200

        # Highlight matches and compute acceptance
        try:
            highlighted = highlight_text(py_regex, text)
        except Exception:
            highlighted = ""
        ...
        # Return an error message but do not crash
        return jsonify(
            {"translated": None, "highlighted": "", "accepted": [], "error": str(e)}
        ), 200
